...
- `llvm-capabilities --check-dir [--json] [--emit-lowering-plan] [--include=GLOB] [--exclude=GLOB] [--fail-on-warning] DIR`
...
Use `bin/pfc llvm-capabilities` for the full supported subset, `bin/pfc llvm-capabilities --json` for machine-readable output, `bin/pfc llvm-capabilities --check [--json] [--fix-suggestions] [--emit-lowering-plan] FILE.ll` to preflight an LLVM file, `bin/pfc llvm-capabilities --check-dir [--json] [--emit-lowering-plan] [--include=GLOB] [--exclude=GLOB] [--fail-on-warning] DIR` to preflight a fixture tree, or `bin/pfc llvm-capabilities --explain FILE.ll` for human-readable lowering guidance. At a high level, the subset supports:
