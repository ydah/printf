...
- Clang tolerance: typed-pointer-style syntax, common value attributes, `getelementptr` no-op flags, trailing metadata, module-level metadata, attributes blocks, `target datalayout`, aliases, single-thread atomic load/store plus no-op `fence`, no-op `llvm.assume.*` / `llvm.lifetime.start/end` / `llvm.sideeffect` / `llvm.donothing` / `llvm.invariant.end` / `llvm.dbg.*` / `#dbg_*`, conservative `llvm.objectsize.*` / `llvm.is.constant.*`, identity `llvm.annotation.*` / `llvm.ptr.annotation.*` / `llvm.expect.*`, and no-op `llvm.global_ctors` / `llvm.global_dtors` metadata globals.
