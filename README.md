...
- Libc surface: `putchar`, `getchar`, `puts`, `strlen`, `strcmp`, `strncmp`, `strchr`, `strrchr`, `strspn`, `strcspn`, `strpbrk`, `memcpy`, `memmove`, `memset`, `memcmp`, `memchr`, and static `printf` formats for integer, character, string, pointer, width, precision, flags, and escaped percent cases.
