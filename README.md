...
- `llvm-capabilities --suggest-next [--json] INPUT.ll|DIR`
...
Use `bin/pfc llvm-capabilities` for the full supported subset, `bin/pfc llvm-capabilities --json` for machine-readable output, `bin/pfc llvm-capabilities --check [--json] [--fix-suggestions] [--emit-lowering-plan] [--validate-schema] [--fail-on=error|warning|none] [--max-warnings=N] FILE.ll` to preflight one file, `bin/pfc llvm-capabilities --check-dir [--json] [--format=json|sarif] [--emit-lowering-plan] [--validate-schema] [--include=GLOB] [--exclude=GLOB] [--fail-on=error|warning|none] [--max-warnings=N] DIR` to preflight a fixture tree, `bin/pfc llvm-capabilities --coverage-report [--json] FILE.ll|DIR` to summarize opcode/diagnostic coverage, `bin/pfc llvm-capabilities --suggest-next [--json] FILE.ll|DIR` to rank implementation candidates, or `bin/pfc llvm-capabilities --explain FILE.ll` for lowering guidance. At a high level, the subset supports:

- Memory: byte-addressed local memory, integer/pointer/aggregate/vector `alloca`/`load`/`store` in main and internal functions, `i128` load/store with high 64-bit preservation, numeric globals, mutable byte-string globals, zero-initialized integer/aggregate external-global stubs, string globals, nested struct/array initializers including `[N x i8] c"..."`, pointer fields, global initializer relocations, compound `getelementptr` including `inrange(...)`, aggregate byte equality/compare helpers, and `llvm.memset.*` / `llvm.memcpy.*` / `llvm.memcpy.inline.*` / `llvm.memmove.*` with common argument attributes.
- Values: `i1`/`i8`/`i16`/`i32`/`i64`, limited `i128` zero/add/sub/bitwise/shift/signed-and-unsigned-compare/select/phi/zext/sext/truncation with high 64-bit preservation, fixed-length integer and pointer vectors, arbitrary constant-mask integer and pointer `shufflevector`, aggregate byte `icmp eq/ne`, aggregate `phi`/`select`, integer arithmetic with no-op `nuw`/`nsw`/`exact`/`disjoint` flags, scalar constant expressions including pointer `ptrtoint` plus pointer `icmp eq/ne` and pointer `select`, casts, pointer tagging, pointer `bitcast`, default-address-space `addrspacecast`, constants, `freeze`, `extractvalue`, `insertvalue`, scalar numeric intrinsics, integer `llvm.vector.reduce.*`, saturating/funnel/bitreverse intrinsics, and `llvm.[su]{add,sub,mul}.with.overflow.*`.
- Control flow: `br`, dense `switch` lowered to C `switch` with bit-width-masked integer case comparisons, scalar, pointer, and limited `i128`/vector `phi`, `ret`, `unreachable`, `nounwind invoke`, nested non-recursive internal calls with integer, pointer, aggregate, `i128`, vector, and void returns, limited indirect calls through known builtin/internal function pointers and constant or dynamic-index function pointer tables, `sret`-style aggregate returns through the first pointer parameter, and `byval` pointer arguments with callee-local copies.
- Clang tolerance: typed-pointer-style syntax, common value attributes, `getelementptr` no-op flags, trailing metadata, module-level metadata, attributes blocks, `target datalayout`, aliases, single-thread atomic load/store plus integer `atomicrmw`/`cmpxchg` including wrap/saturating variants and no-op `fence`, no-op `llvm.assume.*` / `llvm.lifetime.start/end` / `llvm.sideeffect` / `llvm.donothing` / `llvm.invariant.end` / `llvm.dbg.*` / `#dbg_*`, conservative `llvm.objectsize.*` / `llvm.is.constant.*`, identity `llvm.annotation.*` / `llvm.ptr.annotation.*` / `llvm.expect.*`, and no-op `llvm.global_ctors` / `llvm.global_dtors` metadata globals.
- Libc surface: `putchar`, `getchar`, `puts`, `strlen`, `strcpy`, `strncpy`, `strcat`, `strncat`, `strdup`, `strcmp`, `strncmp`, `strchr`, `strrchr`, `strspn`, `strcspn`, `strpbrk`, `atoi`, `strtol` with null `endptr`, `isdigit`, `isalpha`, `isalnum`, `isspace`, `tolower`, `toupper`, `malloc`, `calloc`, no-op `free`, `memcpy`, `memmove`, `memset`, `memcmp`, `memchr`, and static `printf` formats for integer, character, string, pointer, width, precision, flags, and escaped percent cases.

Out-of-scope LLVM features should fail with explicit diagnostics rather than silently compiling. This includes unsupported vector shapes/shuffles, floating-point types, unsupported `i128` operations, unsupported atomic variants, exception handling beyond `nounwind invoke`, varargs, `blockaddress`, unsupported external global declarations, non-zero address spaces, escaped local pointers to unknown functions, and unsupported indirect call targets. JSON preflight diagnostics include `schema_version`, `policy`, `summary`, `severity`, `opcode`, `hint`, `explanation`, `suggestion`, `fix_suggestions`, `docs_url`, `minimal_repro_hint`, and `line_text` fields; `severity` distinguishes blocking `error`, policy-driven `warning`, and accepted backend-equivalent `info` diagnostics. The public schema is `docs/llvm-capabilities.schema.json`, and `--validate-schema` checks emitted results against the in-repo JSON Schema contract. `--emit-lowering-plan` returns structured lowering operations and warning advisories with replacement strategy, risk, runtime-support metadata, and concrete before/after IR examples for external tooling. `--format=sarif` emits SARIF 2.1.0 for code scanning integrations. `--coverage-report` includes top unsupported opcode and intrinsic rankings, and `--suggest-next` turns those rankings into implementation candidates with fixture-candidate and unsupported-reduction context.
